# Fleur Journald 2-Week Assessment

**Server**: fleur.lavnet.net
**Requested**: 5 random samples/day over 2 weeks to evaluate last week's tuning
**Data Source**: systemd journal only (Apache excluded per request)
**Generated**: 2025-12-29

## Critical Finding: Insufficient Journal Retention

**Journal retention limitation discovered**: Journald logs on fleur only go back **6.6 hours** (since 2025-12-29 15:14:14).

**Impact**:
- Cannot assess "two weeks back" as requested
- Cannot evaluate tuning changes from last week
- Cannot identify day-of-week patterns
- Assessment limited to single-day snapshot

**Recommendation**: Configure journal retention before next assessment (see Recommendations section).

## Assessment Results (Available Data)

Statistical analysis of 5 random 1-hour samples from available 6.6-hour window shows Alloy journal filters achieve **80.6% reduction** (median: 89.0%, σ: 15.5%). Raw journal volume averages 5,902 entries/hour (median: 6,594), reduced to 932 filtered entries/hour (median: 771).
...
While the requested 2-week assessment could not be completed due to journal retention limitations, the available data demonstrates:

1. **Strong filter performance**: 80.6% mean efficiency with 89.0% median
2. **Consistent behavior**: Low variance (15.5%) across most samples
3. **Time-sensitive patterns**: Late evening shows different characteristics
4. **Infrastructure gap**: Journal retention must be extended for meaningful trend analysis

**Next Steps**: Configure journal retention to enable the intended 2-week assessment and tuning validation.
